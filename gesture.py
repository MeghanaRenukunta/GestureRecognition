...
# Connect to the rover control server
rover_host =  '192.168.56.1'  # Replace with the IP address or hostname of the rover
rover_port = 5  # Replace with the port number the rover is listening on
...
def calc_bounding_rect(image, landmarks):
    ...
    if (w>250 and h>350):
        ...
        elif x>=150 and x<=250 and y<100:
            print("Forward Movement")
            cv.rectangle(image, (left, top), (right, bottom), (0, 255, 0), 2)
            cv.putText(image, 'Moving Forward', (left, top - 10),
               cv.FONT_HERSHEY_SIMPLEX, 0.9, (0,255, 0), 2)
            detect_gesture(1)
        elif x>=150 and x<=250 and y>200:
            print("Backward Movement")
            cv.rectangle(image, (left, top), (right, bottom), (0, 255, 0), 2)
            cv.putText(image, 'Moving Backward', (left, top - 10),
               cv.FONT_HERSHEY_SIMPLEX, 0.9, (0,255, 0), 2)
            detect_gesture(4)
        elif x<150 and y>=100 and y<=200 :
            print("Left side Movement")
            cv.rectangle(image, (left, top), (right, bottom), (0, 255, 0), 2)
            cv.putText(image, 'Moving Left', (left, top - 10),
               cv.FONT_HERSHEY_SIMPLEX, 0.9, (0,255, 0), 2)
            detect_gesture(3)
        elif x>250 and y>=100 and y<=200:
            print("Right side Movement")
            cv.rectangle(image, (left, top), (right, bottom), (0, 255, 0), 2)
            cv.putText(image, 'Moving Right', (left, top - 10),
               cv.FONT_HERSHEY_SIMPLEX, 0.9, (0,255, 0), 2)
            detect_gesture(2)
    return [x, y, x + w, y + h]
